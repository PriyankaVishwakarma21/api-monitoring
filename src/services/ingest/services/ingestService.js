...
export class IngestService {
    constructor({ eventProducer }) {
        if (!eventProducer) throw new Error("Inguest Service required eventProducer");
    ...
    validateHitData = (hitData) => {
        ...
        if (!validMethods.includes(hitData.method.toUpperCase())) throw new AppError(`Invalid HTTP methods: ${hitData.method}`, 400);

        const statusCode = parseInt(hitData.statusCode, 10);
        if (isNaN(statusCode) || statusCode < 100 || statusCode > 599) throw new AppError(`Invalid status code: ${hitData.statusCode}`, 400);

        const latency = parseFloat(hitData.latency);
        if (isNaN(latency) || latency < 0) throw new AppError(`Invalid  latency: ${hitData.latency}`, 400);
